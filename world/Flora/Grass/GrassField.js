...
export class GrassField extends CGFobject {
  ...
  rebuild() {
    ...
    while (this.blades.length < this.numBlades && attempts < maxAttempts) {
      ...
      if (x * x + z * z > edgeLimit * edgeLimit) continue;
      ...
      this.blades.push({
        x,
        z,
        y: this.terrain ? this.terrain.getHeightAt(x, z) : 0,
    ...
    this.blades.sort((a, b) => a.dead - b.dead); 
  }
  ...
  display(t = 0) {
    ...
    for (const b of this.blades) {
      ...
      this.grassShader.setUniformsValues({ uPhase: b.phase });
      this.scene.pushMatrix();
      this.scene.translate(b.x, b.y, b.z); 
